...
description: "Learn how to integrate Atlas Identity Platform's backend into your Python application"
---

To authenticate your Python server endpoints, you need to send the user's access token in the headers of the request to your server, and then make a request to Atlas Identity Platform's server API to verify the user identity.
...
Atlas Identity Platform provides two methods for authenticating users on your Python server endpoints:
...
2. **REST API Verification**: Makes a request to Atlas Identity Platform's servers to validate the token and retrieve comprehensive user information. Best when you need complete, up-to-date user data.
...
jwks_client = PyJWKClient("https://api.opendex.com/api/v1/projects/<your-project-id>/.well-known/jwks.json")
...
As shown in the [setup guide](../getting-started/setup), make sure you have your Atlas Identity Platform credentials configured:
...
jwks_client = PyJWKClient(f"https://api.opendex.com/api/v1/projects/{stack_project_id}/.well-known/jwks.json")
