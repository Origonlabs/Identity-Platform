...
Before getting started, make sure you have a [Next.js project](https://nextjs.org/docs/getting-started/installation) using the app router, as Atlas Identity Platform does not support the pages router.
...
<Tabs defaultValue="wizard">
  ...
  <TabsContent value="wizard">
    <Steps>
      ...
      Then, create an account on [the Atlas Identity Platform dashboard](https://dashboard.opendex.com/projects), create a new project with an API key, and copy its environment variables into the `.env.local` file of your Next.js project:
  ...
  <TabsContent value="manual">
    ...
    If you are struggling with the setup wizard, please reach out to us on our [Discord](mailto:support@opendex.com) first, where we'll be happy to help you.
    <Steps>
      <Step>
      ### Install npm package
      </Step>
      ...
      If you haven't already, [register a new account on Stack](https://dashboard.opendex.com/handler/sign-up). Create a project in the dashboard, create a new API key from the left sidebar, and copy the project ID, publishable client key, and secret server key into a new file called `.env.local` in the root of your Next.js project:
...
<Tabs defaultValue="wizard">
  ...
  <TabsContent value="wizard">
    <Steps>
      ...
      Then, create an account on [the Atlas Identity Platform dashboard](https://dashboard.opendex.com/projects), create a new project with an API key, and copy its values into the `stack/client.ts` file created by the wizard.
  ...
  <TabsContent value="manual">
    <Steps>
      ...
      ```bash title="Terminal"
      npm install @opendex/react
      ```

      <Step>
      ### Create API keys
      </Step>
      If you haven't already, [register a new account on Stack](https://dashboard.opendex.com/projects), create a project in the dashboard, create a new API key from the left sidebar, and copy the project ID and publishable client key. Store them based on your project setup (environment variables or directly in the client file during development).
...
<Tabs defaultValue="wizard">
  ...
  <TabsContent value="wizard">
    <Steps>
      ...
      Then, create an account on [the Atlas Identity Platform dashboard](https://dashboard.opendex.com/projects), create a new project with an API key, and copy its values into the `stack/server.ts` or `stack/client.ts` file.
...
Our recommended way to use Atlas Identity Platform with Python is with the [REST API](../rest-api/overview.mdx). It provides a fully documented way to interact with Atlas Identity Platform from any Python framework, including Flask, FastAPI, and Django.

For the purpose of this guide, we will use the `requests` library to make HTTP requests to the Atlas Identity Platform API. If you haven't already, you can install it in your environment with `pip install requests`.

<Steps>
  ### Create API keys

  First, create an account on [the Atlas Identity Platform dashboard](https://dashboard.opendex.com/projects), and copy your project ID, publishable client key, and secret server key into a safe place (eg. environment variables).
  ...
  Next, create a helper function to make requests to the Atlas Identity Platform API:
  ...
  In Atlas Identity Platform's JavaScript SDK, you can retrieve the access token [from the `stackClientApp` object](/sdk/types/user#currentusergetauthjson). Then, you can use said access token to make requests to Atlas Identity Platform:
