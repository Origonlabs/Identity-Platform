...
This guide explains how to set up X (formerly Twitter) as an authentication provider with Atlas Identity Platform. X OAuth 2.0 allows users to sign in to your application using their X account.
...
<Steps>
  ...
  11. Enable **OAuth 2.0** and set the following details:
      - **Type of App**: Web App
      - **Callback URL / Redirect URL**: `https://api.opendex.com/api/latest/auth/oauth/callback/x`
      - **Website URL**: Your website's URL
  12. Under **App permissions**, select your scopes.
  13. Click **Save** to apply your changes.
  </Step>
  <Step>
  ### Enable X OAuth in Atlas Identity Platform
