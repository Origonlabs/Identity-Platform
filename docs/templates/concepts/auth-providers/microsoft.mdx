...
This guide explains how to set up Microsoft as an authentication provider with Atlas Identity Platform. Microsoft OAuth allows users to sign in to your application using their Microsoft account.

<Info>
For Development purposes, Atlas Identity Platform uses shared keys for this provider. Shared keys are automatically created by Stack, but show Stack's logo on the OAuth sign-in page.
...
<Steps>
  ...
  6. In the **Redirect URI** section, select **Web** as the platform and enter `https://api.opendex.com/api/v1/auth/oauth/callback/microsoft`
  ...
  <Step>
  ### Enable Microsoft OAuth in Atlas Identity Platform
  
  1. On the Atlas Identity Platform dashboard, select **Auth Methods** in the left sidebar.
...
- Join our [Discord](mailto:support@opendex.com)
