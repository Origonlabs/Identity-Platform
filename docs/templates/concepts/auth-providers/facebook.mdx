...
This guide explains how to set up Facebook as an authentication provider with Atlas Identity Platform. Facebook OAuth allows users to sign in to your application using their Facebook account.
...
<Steps>
  <Step>
    ...
    10. On the Permissions tab, next to **email**, select **Add** to allow Atlas Identity Platform to read your user's primary email address.
    11. In the left sidenav, under **Facebook Login**, select **Settings**.
    12. In the **Client OAuth settings** section, in the **Valid OAuth Redirect URIs** field, add `https://api.opendex.com/api/v1/auth/oauth/callback/facebook`
  ...
  <Step>
    ### Enable Facebook OAuth in Atlas Identity Platform
