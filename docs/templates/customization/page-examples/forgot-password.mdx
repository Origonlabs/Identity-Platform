...
The `ForgotPassword` component provides a complete form for users to request a password reset email. When a user submits their email, Atlas Identity Platform will send them an email with a link to reset their password.

```tsx  
'use client';
import { ForgotPassword } from "@opendex/stack";
...
2. Configure Atlas Identity Platform to use your custom route in your `stack/server.ts` file:
...
This ensures that links to the forgot password page will direct users to your custom implementation. When a user submits their email, Atlas Identity Platform will send them an email with a link to the password reset page configured in your application.
