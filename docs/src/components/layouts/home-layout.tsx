...
// Atlas Identity Platform Logo Component
function StackAuthLogo() {
  return (
    <Link href="https://opendex.com" className="flex items-center gap-2.5 text-fd-foreground hover:text-fd-foreground/80 transition-colors">
      <svg
        width="30"
        height="24"
        viewBox="0 0 200 242"
      ...
      <span className="font-medium text-[15px]">Atlas Identity Platform</span>
    </Link>
  );
}
    ...
    <>
      ...
      <div className={`fixed top-4 left-1/2 -translate-x-1/2 transition-all duration-300 ${isScrolled ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'} ${isChatExpanded ? 'z-[80]' : 'z-50'}`}>
        <div className="flex items-center gap-3 px-4 py-2 bg-fd-background/95 backdrop-blur border border-fd-border rounded-full shadow-lg supports-[backdrop-filter]:bg-fd-background/80">
          ...
          <Link href="https://opendex.com" className="flex items-center text-fd-foreground hover:text-fd-foreground/80 transition-colors">
            <svg
              width="40"
              height="32"
              viewBox="0 0 200 242"
