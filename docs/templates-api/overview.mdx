---
title: API Reference
description: Complete REST API documentation for Atlas Identity Platform
full: true
---

# REST API & Webhooks

Atlas Identity Platform offers a REST API for backends & frontends of any programming language or framework. This API is used to authenticate users, manage user data, and more.
...
```http
curl https://api.opendex.com/api/v1/ \
...
<AccordionGroup>
  ...
  <Accordion title="How do I handle API errors?">
    Atlas Identity Platform API returns standard HTTP status codes. Common error responses include:
  ...
  <Accordion title="Are there rate limits?">
    Yes, Atlas Identity Platform implements rate limiting to ensure fair usage and system stability. Rate limits vary by endpoint and access type. When you exceed the rate limit, you'll receive a `429 Too Many Requests` response with headers indicating when you can retry.
  </Accordion>
</AccordionGroup>
...
- Visit our [Concepts](/docs/next/concepts) section to understand Atlas Identity Platform fundamentals  
- Join our [Discord community](mailto:support@opendex.com/) for support and discussions
