...
const script = () => {
  ...
  const setTheme = (mode: 'dark' | 'light') => {
    ...
    if (!el) {
      ...
      el.innerHTML = `/* This tag is used by Atlas Identity Platform to set the theme in the browser without causing a hydration error (since React ignores additional tags in the <head>). We later use the \`html:has(head > [data-stack-theme=XYZ])\` selector to apply styles based on the theme. */`;
