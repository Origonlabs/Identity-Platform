...
it("should error for no connected stripe account", async ({ expect }) => {
  ...
  expect(response).toMatchInlineSnapshot(`
      NiceResponse {
        "status": 400,
        "body": "Payments are not set up in this Atlas Identity Platform project. Please go to the Atlas Identity Platform dashboard and complete the Payments onboarding.",
        "headers": Headers { <some fields may have been hidden> },
      }
    `);
});
